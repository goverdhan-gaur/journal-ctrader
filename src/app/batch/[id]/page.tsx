...
export default function Home() {
    const { id } = useParams();
    const trade = useTradesStore(state => state.getTradeById(id ? (Array.isArray(id) ? id[0] : id) : ""));
    const tradesData = useTradesStore(state => state.tradesData);

    // Prevent infinite loading if Zustand hasn't loaded from localStorage yet
        ...
        <div className="container mx-auto p-4">
            {trade ? (
                <div>
                    ...
                    <TradeList trades={trade.tradeSummary} id={Array.isArray(id) ? id[0] : id}/>
                </div>
            ) : (
